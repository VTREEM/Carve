...
namespace carve {
  namespace interpolate {
    ...
    class Interpolator {
    public:
      virtual void interpolate(const carve::poly::Face *new_face,
    ...
    protected:
      struct fv_hash {
        size_t operator()(const std::pair<const carve::poly::Face *, unsigned> &v) const {
          return size_t(v.first) ^ size_t(v.second);
        }
      };

      typedef std::unordered_map<const carve::poly::Vertex *, attr_t, carve::poly::hash_vertex_ptr> attrvmap_t;
      typedef std::unordered_map<std::pair<const carve::poly::Face *, unsigned>, attr_t, fv_hash> attrmap_t;

      attrmap_t attrs;

    public:
      bool hasAttribute(const carve::poly::Face *f, unsigned v) {
        return attrs.find(std::make_pair(f, v)) != attrs.end();
      }

      attr_t getAttribute(const carve::poly::Face *f, unsigned v, const attr_t &def = attr_t()) {
      ...
      void setAttribute(const carve::poly::Face *f, unsigned v, const attr_t &attr) {
      ...
      virtual void interpolate(const carve::poly::Face *new_face,
        ...
        for (size_t i = 0; i < new_face->vertices.size(); ++i) {
          const carve::poly::Vertex *vertex = new_face->vertices[i];
          ...
          if (b != base_attrs.end()) {
            ...
            attr_t attr = interp(orig_face->vertices,
                                 carve::poly::p2_adapt_project(orig_face->project),
                                 vertex_attrs,
                                 p.x,
                                 p.y);
    ...
    protected:
      struct f_hash {
        size_t operator()(const carve::poly::Face * const &f) const {
          return size_t(f);
        }
      };

      typedef std::unordered_map<const carve::poly::Face *, attr_t, f_hash> attrmap_t;

      attrmap_t attrs;

    public:
      bool hasAttribute(const carve::poly::Face *f) {
        return attrs.find(f) != attrs.end();
      }

      attr_t getAttribute(const carve::poly::Face *f, const attr_t &def = attr_t()) {
      ...
      void setAttribute(const carve::poly::Face *f, const attr_t &attr) {
        attrs[f] = attr;
      }
